...
func WithdrawStatusScanJob()  {
	fmt.Printf("TrxWithdrawStatusScanJob任务开始")
	var offset = 0
	var limit = 100
	var count = 6*30
	...
	for {
		...
		db.Debug().Model(&models.WithdrawRecord{}).Where("type = ? AND status = ?", "TRX_USDT", 0).Offset(offset).Limit(limit).Find(&withdraws)

		if len(withdraws) > 0 {
			for _, withdraw := range withdraws {
				tx := db.Begin()
				if withdraw.Count < count {
					b, err := client.GetTransactionReceipt(withdraw.Tx)
					//if err != nil {//出错直接失败
	...
	fmt.Printf("TrxWithdrawStatusScanJob任务结束")
}
